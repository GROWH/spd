    ...
    upload: function (fn) {
        ...
        this.fp.form.submit({
            ...
            success: function (form, action) {
                ...
                var obj = {
                    title: this.fp.form.findField("title").getValue(),
...
AddFriendWindow = Ext.extend(Ext.Window, {
    ...
    initComponent: function () {
        this.buttons = [{text: "发送好友申请", handler: this.addFriend, scope: this}, {
        ...
        this.fp = new Ext.form.FormPanel({
            ...
            border: false,
            labelWidth: 60,
            items: [{xtype: "hidden", id: "userId"}, {id: "headInfo", border: false}, {
                xtype: "textarea",
                name: "remark",
                fieldLabel: "备注信息",
                width: 200
...
FindFriendWindow = Ext.extend(Ext.Window, {
    ...
    findFriend: function () {
        ...
        if (!f.findField("name").getValue() && !f.findField("sex").getValue() && !f.findField("email").getValue() && !f.findField("im").getValue() && !f.findField("speciality").getValue() && (!f.findField("cityId").getValue() || f.findField("cityId").getValue() <= 0)) {
            Ext.Msg.alert("提示", "至少要输入一项查询条件", function () {
    ...
    initComponent: function () {
            ...
            initComponent: function () {
                this.tools = [{
                    id: "refresh", handler: function () {
                        if (this.store.baseParams.searched) this.store.reload();
                    }, scope: this
                }];
        ...
        this.menu = new Ext.menu.Menu({
            items: [{text: "发送消息", handler: this.chat, scope: this},
        ...
        this.fp = new Ext.form.FormPanel({
            ...
            buttons: [{text: "搜索", handler: this.findFriend, scope: this}, {
                text: "取消", handler: function () {
                    this.hide();
                }, scope: this
            }]
        ...
        this.on("render", function () {
            ...
            this.userList.grid.on("rowclick", function (g, rowIndex, e) {
                this.currentRecord = this.userList.store.getAt(rowIndex);
            }, this)
        }, this)
    }
    ...
    config: function () {
        if (!this.configWin)
            this.configWin = new Ext.Window({
                ...
                buttons: [{text: "确定", handler: this.saveConfig, scope: this}, {
                    text: "取消", handler: function () {
                        this.configWin.hide();
                    }, scope: this
                }]
    ...
    showSystemMessage: function (message) {
        ...
        if (!this.sysWin) {
            this.sysWin = new Ext.Window({
                ...
                title: "系统消息",
                html: "SomeThing",
                iconCls: "erpwin-announce-icon",
                closeAction: "hide"
            });
            this.sysWin.on("show", function (win) {
                win.hide.defer(60000, win, [true])
            });
        }
        else {
            this.sysWin.el.moveTo(bx, by);
        }
        var win = this.sysWin;
        win.show();
                ...
                if (list && list.length) {
                    for (var i = list.length - 1; i >= 0; i--) {
                        ...
                        var obj = {
                            ...
                            date: o.inputTime,
                            user: o.sender,
                            cls: (OnlineMessageManager.me.id == o.sender.id ? "myMsg" : "yourMsg")
                        };
                        this.addMessage(obj);
                    }
                }
    ...
    initComponent: function () {
        this.tbar = [this.reciver.name,
            {
                text: "查看资料",
                cls: "x-btn-text-icon",
                icon: "images/qq/zoom.gif",
                handler: OnlineMessageManager.showUserInfo.createDelegate(OnlineMessageManager, [this.reciver.id])
            }
        ...
        this.add({
            ...
            tbar: [{
                text: "聊天记录",
                cls: "x-btn-text-icon",
                icon: "images/qq/loadHistory.gif",
            ...
            },
                {
                    text: "清空记录",
                    cls: "x-btn-text-icon",
                    icon: "images/qq/cleanHistory.gif",
                ...
                },
                {
                    text: "传递附件",
                    cls: "x-btn-text-icon",
                    icon: "images/qq/updown.gif",
                    handler: OnlineMessageManager.sendAttach.createDelegate(this, [this.reciver.id])
                }, "是否自动关闭",
                {
                    xtype: "checkbox",
            ...
            layout: "fit",
            items: this.editor,
            buttons: [{text: "发送", handler: this.sendMessage, scope: this}, {
...
OnlineChatWindow = Ext.extend(Ext.Window, {
    ...
    initComponent: function () {
        this.tbar = [
            {
                text: "刷新",
                cls: "x-btn-text-icon",
                icon: "images/operation/refresh.gif",
            ...
            "-",
            {
                ...
                hidden: true,
                icon: "images/qq/zoom.gif",
                handler: OnlineMessageManager.findFriend
            },
            {
                text: "设置",
                cls: "x-btn-text-icon",
        ...
        this.menu = new Ext.menu.Menu({
            items: [{
                text: "发送消息", handler: function () {
                    this.chat(this.currentNode);
                }, scope: this
            },
                {
                    text: "查看资料", handler: function () {
                    OnlineMessageManager.showUserInfo(this.currentNode.id)
                }, scope: this
                },
                {
                    text: "加为好友", hidden: true, handler: function () {
                    this.addFriend(this.currentNode)
                }, scope: this
                },
                {
                    text: "查看动态", hidden: true, handler: function () {
                    this.addFriend(this.currentNode)
                }, scope: this
                },
                {
                    text: "用户主页", hidden: true, handler: function () {
        ...
        this.add({
            title: "员工信息", iconCls: "icon-online",
            layout: "fit",
            //collapsed:true,
            border: false,
            items: {
        ...
        this.add({
            title: "最近联系人", iconCls: "icon-timelist",
            layout: "fit",
            collapsed: true,
            border: false,
            items: {
    ...
    openMessage: function (message) {
        ...
        if (message.content) {
            var obj = {
                ...
                user: {name: message.sender, id: message.sender},
                cls: message.sender == OnlineMessageManager.me.name ? "myMsg" : "yourMsg"
            };
            msgWin.addMessage(obj);
...
MettingMessageWindow = Ext.extend(Ext.Window, {
    ...
    initComponent: function () {
        ...
        this.tbar = ["<img src='images/qq/techer.gif'/>", "主讲:", this.teacher, {
            text: "查看详情",
            cls: "x-btn-text-icon",
            icon: "images/qq/zoom.gif"
        }, "-", "<img src='images/core/start.gif'/>", "开始时间:", this.beginTime, "-", "<img src='images/core/stop.gif'/>", "结束时间:", this.endTime, "->", {
            text: "申请发言",
            cls: "x-btn-text-icon",
            icon: "images/qq/ren1.gif",
            disabled: true
        }, {text: "退出课堂", cls: "x-btn-text-icon", icon: "images/qq/exit.gif", handler: this.exit, scope: this}];
        ...
        this.allPerson = new Ext.Toolbar.Button({
            text: "所有人", disabled: true, handler: function () {
