...
public class FileUtils {
    private static final JRPlatLogger LOG = JRPlatLoggerFactory.getJRPlatLogger(FileUtils.class);
     ...
     * @return 新文件
     */
    public static File createAndWriteFileWithAbsolutePath(String path, byte[] data) {
        try {
            File file = new File(path);
            if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs();
    ...
    public static List<String> getTextFileContent(String path) {
        try {
            ...
            if (!path.contains("jrplat.licence")) {
                LOG.error("文件不存在", ex);
            }
        }
        //Null Object设计模式
