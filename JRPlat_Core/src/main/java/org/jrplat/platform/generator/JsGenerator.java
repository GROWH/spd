...
import org.jrplat.platform.model.ModelFieldData;
...
public class JsGenerator extends Generator {
    ...
    public static String getContent(String modulePath, String module) {
        ...
        //Renderingore字段存在一个问题，查询时，查不到数据，
        //因此增加一个类型DisplayIgnore类型，这种类型只是在表更不显示    ---cyj
//        List<ModelFieldData> attrs=model.getAllRenderModelAttr();
        List<ModelFieldData> attrs = model.getAllDisplayAttrs();
        ...
        //搜索，分为两列
        List<ModelFieldData> searchableAttrs = model.getAllDisplayAttrs();
