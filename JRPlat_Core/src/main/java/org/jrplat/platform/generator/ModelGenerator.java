...
public class ModelGenerator extends Generator {
            ...
            for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
                ...
                try {
                    ...
                    for (int rowNumber = 7; rowNumber < rows; rowNumber++) {
                        ...
                        // 是否显示到页面表格 --- add cyj
                        oneCell = oneRow.getCell(10);
                        attr.setDisTolist(true);
                        if (oneCell != null) {
                            boolean cellValue = oneCell.getBooleanCellValue();
                            attr.setDisTolist(cellValue);
                        }

                        //是否允许为空
                        oneCell = oneRow.getCell(11);
                        attr.setIfNull(true);
                        if (oneCell != null) {
                            boolean cellValue = oneCell.getBooleanCellValue();
                            attr.setIfNull(cellValue);
                        }
                        // add end
    ...
    final public static class Attr {
        ...
        private boolean disTolist = true;
        ...
        public boolean isDisTolist() {
            return disTolist;
        }

        public void setDisTolist(boolean disTolist) {
            this.disTolist = disTolist;
        }

        @Override
        public String toString() {
            return "Attr{" + "name=" + name + ", type=" + type + ", length="
                    + length + ", des=" + des + ", searchable=" + searchable
                    + ", map=" + map + ", attrRef=" + attrRef
                    + ", renderIgnore=" + renderIgnore + ", dic=" + dic
                    + ", dicName=" + dicName + ",disTolist=" + disTolist + ",ifNull=" + ifNull + '}';
        }
    }
