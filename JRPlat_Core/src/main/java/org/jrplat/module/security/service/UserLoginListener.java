...
public class UserLoginListener implements HttpSessionAttributeListener, HttpSessionListener {
    private static final JRPlatLogger LOG = JRPlatLoggerFactory.getJRPlatLogger(UserLoginListener.class);
        ...
        if (se.getValue() instanceof SecurityContextImpl && loginMonitor) {
            ...
            if (null != user) {
                ...
                if (logs.get(user.getUsername()) == null) {
                    ...
                    if (se.getSession().getAttribute("userAgent") == null) {
                        userLogin.setUserAgent("From TMS App");
                    } else {
