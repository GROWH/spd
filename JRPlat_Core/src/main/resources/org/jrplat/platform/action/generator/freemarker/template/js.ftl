    ...
    return {
        show: function (infor) {
            var elements = [
                {
                    name: "仓库名称",
                    field: "rnwarehouse.StorehouseName",
                    opt: "like"
                }, {
                    name: "收货类型",
                    field: "rnReceivingType.name",
                    opt: "eq",
                    store: rnReceivingTypeStore
                }
            ];
...
<#list attrs as attr>
    <#if attr_has_next>
        <#if attr.manyToOne && attr.manyToOneRef != ''>
                        {header: "${attr.chinese}", dataIndex: '${attr.english}_${attr.manyToOneRef}'},
        <#else>
                        {header: "${attr.chinese}", dataIndex: '${attr.english}'},
        </#if>
    <#else>
        <#if attr.manyToOne && attr.manyToOneRef != ''>
                        {header: "${attr.chinese}", dataIndex: '${attr.english}_${attr.manyToOneRef}'}
        <#else>
                ...
                GridBaseModel.show(contextPath, namespace, action, pageSize, gridObj.fields, gridObj.columns, commands, tips, callbacks);
                GridBaseModel.onRowClick = function(){};
            }
        }
    } ();
